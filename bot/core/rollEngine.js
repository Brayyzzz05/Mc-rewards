...
export async function rollReward(userId, mcName) {
  ...
  // Combine global luck (settings) with the user's personal luck multiplier (from shop)
  ...
  return {
    tier: fallback.tier,
    cmd: fallback.cmd.replaceAll("{player}", mcName),
    luckUsed: luck
  };
}
